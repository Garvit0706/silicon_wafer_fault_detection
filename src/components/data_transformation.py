...
class DataTransformation:
    ...
    def get_data_transformer_object(self):
        try:
            ...
            logging.info("Preprocessor is running")
            return preprocessor
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            logging.info("Loading testing data from: %s", test_path)
            test_df = pd.read_csv(test_path)
 
            preprocessor = self.get_data_transformer_object()

            target_column_name = "Good/Bad"
            #target_column_mapping = {'+1': 1, '-1': 0}
            ...
            logging.info("Input features transformed")

            smt = SMOTETomek(sampling_strategy="minority")
            logging.info("Applying SMOTETomek")
